...
public class AccountChooserButton extends LinearLayout implements OnClickListener {
		...
		if(service != null) {
			...
			for (final SipProfile account : accountsList) {
				...
				if(accountStatusDisplay.availableForCalls) {
					//quickAction.addItem(getResources().getDrawable(WizardUtils.getWizardIconRes(account)), account.display_name, new OnClickListener() yuva {
					quickAction.addItem(getResources().getDrawable(WizardUtils.getWizardIconRes(account)), "Sonetel", new OnClickListener() {
						public void onClick(View v) {
							setAccount(account);
							quickAction.dismiss();
						}
	...
	public void setAccount(SipProfile aAccount) {
		...
		if (account == null) {
			...
			//textView.setText(account.display_name); yuva
			textView.setText("Sonetel");
