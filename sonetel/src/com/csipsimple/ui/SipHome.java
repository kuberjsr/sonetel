...
import com.csipsimple.ui.prefs.SonetelSettings;
...
//	protected static final int PICKUP_PHONE = 0;
	...
	private Intent dialerIntent,calllogsIntent, messagesIntent, contactDialer;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		contactDialer = new Intent(this,Sonetel_Contact_Dialer.class);
		...
		//if(CustomDistribution.showContactTab()) {
		addTab(TAB_CONTACTS, getString(R.string.contacts_tab_name_text), R.drawable.ic_tab_unselected_contacts, R.drawable.ic_tab_selected_contacts, contactDialer);
		//}
			...
			if(nbrOfAccount == 0) {
				...
				if(account != null) {
					if(account.id == SipProfile.INVALID_ID) {
						...
						/** Sonetel start*/
						//startActivity(new Intent(this, AccountsList.class));//yuva
						/** Sonetel end */
					}
				}else {
	...
	private void selectTabWithAction(Intent intent) {
		if(intent != null) {
			...
			else if(SipManager.ACTION_CONTACTS.equalsIgnoreCase(callAction)){
				if(CustomDistribution.showContactTab()) {
	...
	private void populateMenu(Menu menu) {
		...
		if(distribWizard != null) {
			/**Sonetel start */
			menu.add(Menu.NONE, DISTRIB_ACCOUNT_MENU, Menu.NONE, distribWizard.label + " account").setIcon(distribWizard.icon);//yuva
			/**Sonetel end */
		}
