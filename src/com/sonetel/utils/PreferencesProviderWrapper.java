...
public class PreferencesProviderWrapper {
    ...
    public boolean isValidConnectionForOutgoing() {
        if (getPreferenceBooleanValue(PreferencesWrapper.HAS_BEEN_QUIT, false)) {
            return isValidConnectionForOutgoing(true);
                      // Don't go further, we have been explicitly stopped	220	    
    ...
    public String getCallthruNumber() {
        String prefsValue = SipConfigManager.getPreferenceStringValue(context,
                SipConfigManager.CALLTHRU_NUMBER);
...
//			if(pinfo != null) {
//				userAgent +=  " " + android.os.Build.DEVICE + "-" + Compatibility.getApiLevel()+"/r"+pinfo.versionCode;
//			}
//		}
//		return userAgent;
//	}
...
//>>>>>>> .r2200
    ...
    public String getUserAgent(Context ctx, int ver) {
        String userAgent = getPreferenceStringValue(SipConfigManager.USER_AGENT);
        if (userAgent.equalsIgnoreCase(CustomDistribution.getUserAgent())) {
            ...
            if (pinfo != null) {
            	if(ver == 0)
            	{
            		...
            		userAgent = android.os.Build.BRAND + "-" + android.os.Build.DEVICE + "-" + android.os.Build.VERSION.RELEASE + "-" + Compatibility.getApiLevel()
                    + "-" + pinfo.versionCode;
            }
        }
        return userAgent;
