...
import com.sonetel.api.SipMessage;
...
public class DBAdapter {
	...
	//	public static final String ACCESS_NUMS_TABLE_NAME = "access_numbers";
		//public SQLiteDatabase SQLiteObj;
		
		private static final String TABLE_ACCESS_NUMBERS_CREATE = "CREATE TABLE IF NOT EXISTS "
			+ SipProfile.ACCESS_NUMS_TABLE_NAME
			+ " ("
				+ AccessNumbers.FIELD_ID					+ " INTEGER PRIMARY KEY AUTOINCREMENT,"
				+ AccessNumbers.COUNTRY_ID				+ " TEXT,"
				+ AccessNumbers.ACCESS_NO					+ " TEXT"
			+");";
		...
		private static final String TABLE_ACCOUNT_CREATE = "CREATE TABLE IF NOT EXISTS "
			...
			+ " ("
                ...
                + SipProfile.FIELD_ENTID					 + " TEXT,"
                + SipProfile.FIELD_EXT						 + " TEXT,"
                + SipProfile.FIELD_PIN						 + " TEXT,"
                + SipProfile.FIELD_MOBILE_NBR			     + " TEXT,"
                + SipProfile.FIELD_USRID			 		 + " TEXT,"
//=======
//                + SipProfile.FIELD_ANDROID_GROUP             + " TEXT,"
		...
		public void onCreate(SQLiteDatabase db) {
			...
			db.execSQL(TABLE_ACCESS_NUMBERS_CREATE);
	...
	public SipProfile getAccount(long accountId) {
		...
		return SipProfile.getProfileFromDbId(context, accountId, DBProvider.ACCOUNT_FULL_PROJECTION);
	}
    
	private static final String[] ACCESS_NUMBERS_PROJECTION = new String[]
    {
   		AccessNumbers.FIELD_ID,
		AccessNumbers.COUNTRY_ID,
		AccessNumbers.ACCESS_NO
    };
    
	public String getCallthruNbr(String CntryID, Context ctxt)
	{
		String accessNbr = new String ();
		//DBAdapter db = new DBAdapter(context);
		//if(!isOpen())
			//db = open();
		String[] CountryId ={CntryID};
		if(CntryID != null)
		{
			Cursor acNumber = null;
			
			//SQLiteDatabase db = databaseHelper.getReadableDatabase();
			//db.databaseHelper.getReadableDatabase();
			//acNumber = databaseHelper.getReadableDatabase().query(SipProfile.ACCESS_NUMS_TABLE_NAME, new String[]{"access_no"},"country_id = "+CntryID, null,null,null,null);
			//acNumber = databaseHelper.getWritableDatabase().query(SipProfile.ACCESS_NUMS_TABLE_NAME, new String[]{"access_no"},"country_id = ?", CountryId,null,null,null);
			acNumber = ctxt.getContentResolver().query(SipProfile.ACCESS_URI, new String[]{"access_no"},"country_id = ?", CountryId,null);
			
			if (acNumber != null && acNumber.moveToFirst()) {
				// Read call log specifics
				accessNbr = acNumber.getString(0);
			}
			else
				accessNbr =  null;
		}
