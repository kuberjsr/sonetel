...
import com.sonetel.widgets.RegistrationNotification;
...
public class SipNotifications {
	...
	public synchronized void notifyNotRegisteredAccounts(ArrayList<SipProfileState> activeAccountsInfos, boolean showNumbers) {
		if (!isServiceWrapper) {
			Log.e(THIS_FILE, "Trying to create a service notification from outside the service");
			return;
		}
		//int icon = R.drawable.ic_stat_sipok;
		int icon = R.drawable.not_ok;
		CharSequence tickerText = context.getString(R.string.service_ticker_unregistered_text);
		long when = System.currentTimeMillis();
		 ...
		 Builder nb = new NotificationCompat.Builder(context);
			...
			contentView.addInActiveAccountInfos(context, activeAccountsInfos);
      ...
      //  if(account.pin == null || account.pin.equalsIgnoreCase(""))
		//{
			...
			Toast.makeText(context,"Unable to configure automated callthru. You may be need to enter your credentials", Toast.LENGTH_LONG).show();
		}
