...
public class RegistrationNotification extends RemoteViews {
    ...
    public void addAccountInfos(Context context, ArrayList<SipProfileState> activeAccountsInfos) {
        ...
        for (SipProfileState accountInfo : activeAccountsInfos) {
            ...
            if (i < cells.length) {
                ...
                if (wizardInfos != null) {
                    CharSequence dName = "Sonetel";// accountInfo.getDisplayName();
    ...
    public void addInActiveAccountInfos(Context context, ArrayList<SipProfileState> activeAccountsInfos) {
		int i = 0;
		for(SipProfileState accountInfo : activeAccountsInfos ) {
		//	if(i<cells.length) {
				setViewVisibility(cells[0], View.VISIBLE);
		//		WizardInfo wizardInfos = WizardUtils.getWizardClass(accountInfo.getWizard());
		//		if(wizardInfos != null) {
					//setImageViewResource(icons[i], wizardInfos.icon);
					setImageViewResource(icons[i],R.drawable.not_ok);//setImageResource(R.drawable.ic_wizard_sonetel);
					//setImageViewResource(icons[i], wizardInfos.icon);
					//setTextViewText(texts[i], accountInfo.getDisplayName());
					//setTextViewText(texts[i], "Sonetel VoIP: "+accountInfo.getStatusCode()+" "+accountInfo.getStatusText());
					setTextViewText(texts[i], "Sonetel VoIP: "+ accountInfo.getStatusCode()+ " "+ getSipMsgDescription(accountInfo));
			//	}
		//		i++;
			//}
		}
    ...
    public String getSipMsgDescription(SipProfileState accountInfo)
	{
		if(accountInfo.getStatusCode() == 401)
			return "Invalid user name/password";
		else if(accountInfo.getStatusCode() == 500)
			return "Service error";
		else if(accountInfo.getStatusCode() == 404)
			return "User not found";
		else
			return accountInfo.getStatusText();
